...
import { getAvailableStudents } from '../../services/api';
...
const CoachChatPage = () => {
      ...
      try {
        ...
        setTimeout(() => {
          if (streamChatClient.userID) {
            console.log('✅ Coach Stream Chat client ready with user:', streamChatClient.userID);
            setClientReady(true);
          } else {
            console.log('❌ Coach Stream Chat client not properly connected');
            setError('Failed to connect to chat service');
          }
        }, 1000);
        
        // Get available students (coaches can see students who have booked sessions)
        const studentsData = await getAvailableStudents(authToken);
        const uniqueStudents = studentsData.coaches
          .map((coach: any) => ({
            id: coach.id,
  ...
  const handleStartChat = async (student: any) => {
    try {
      const channelId = `chat-${userId}-${student.id}`;
      const channel = streamChatClient.channel('messaging', channelId, {
        members: [userId!, student.id],
      });
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {channels.length > 0 ? (
                ...
                >
                  ...
                  <View style={styles.conversationContent}>
                    <Text style={styles.conversationName}>
                      {channel.data?.studentName || otherMember?.user?.name || otherMember?.name || 'Student'}
                    </Text>
          ...
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyStateSubtitle}>Start a chat with a student to begin messaging!</Text>
          </View>
        )}
        ...
        {channels.filter(channel => channel.state.members && Object.keys(channel.state.members).length > 2).length > 0 ? (
          ...
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyStateSubtitle}>Create group sessions to see them here!</Text>
          </View>
        )}
      </ScrollView>
      ...
      <ChatModal
        ...
        chatPartnerName={selectedChannel?.data?.studentName || selectedChannel?.data?.name || 'Student'}
      />
    </View>
  );
};
